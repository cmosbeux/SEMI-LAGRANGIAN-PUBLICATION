...
FUNCTION SourceDamage (Model, nodenumber, D) RESULT(Source)
   ...
   REAL (KIND=dp) :: SigmaI, SigmaII, SigmaIII, Chi, B, sigmath, pwater
  ...
  ! Need Cauchy Stress and Deviatoric Stress 
   ...
   !(very large strain rate induces by imposted null velocities on the boundaries)
   IF ( abs(SigmaI) > 1.0_dp) THEN
     Chi = 0.0
   ELSE
